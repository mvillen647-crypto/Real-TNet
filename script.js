...
async function startScan() {
  ...
  btn.disabled = true;
  result.innerHTML = "🔄 Hatua 1: Inapakia picha kwenye wingu...";
  ...
  try {
    ...
    // 2. Upload picha kwenye Cloudinary kupitia API yako
    ...
    if (!uploadData.success) throw new Error("Imeshindwa kupakia picha.");

    result.innerHTML = "🔄 Hatua 2: Inachambua picha kwa kutumia AI...";

    // 3. Scan picha kwa kutumia URL kutoka Cloudinary na Hifadhi Firebase
    const scanRes = await fetch("/api/scan", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        imageUrl: uploadData.url, // Tunatuma URL badala ya base64
        type: activeScanType
      })
    });
    ...
    if (resultData.success) {
      ...
      const humanPercent = ((1 - (ai.type?.ai_generated || 0)) * 100).toFixed(0);
      const barColor = humanPercent >= 50 ? "#4CAF50" : "#F44336";

      result.innerHTML = `
        <div class="result-card-inner">
          <h3>✅ Scan Imekamilika!</h3>
          <div class="bar-container">
            <div class="bar-fill" style="width: ${humanPercent}%; background-color: ${barColor};"></div>
          </div>
          <p style="text-align:center; font-weight:bold;">${humanPercent}% Human | ${100 - humanPercent}% AI</p>
          <p><b>Hali:</b> ${ai.nudity?.none > 0.9 ? "Salama" : "Tahadhari"}</p>
        </div>
      `;
    } else {
