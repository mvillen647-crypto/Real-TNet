...
async function startScan() {
  ...
  // 1. Angalia kama kuna picha imechaguliwa (kwa Image mode)
  ...
  // 2. Disable button ili mtumiaji asibonyeze mara mbili
  btn.disabled = true;
  result.innerHTML = "🔄 Scanning... tafadhali subiri (Hii inaweza kuchukua sekunde kadhaa)...";

  try {
    // 3. Andaa picha (Convert to base64)
    let imageData = "";
    ...
    // 4. Tuma data kwenye Vercel API
    const response = await fetch("/api/scan", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        image: imageData,
        type: activeScanType // Tuma pia aina ya scan
      })
    });
    ...
    if (resultData.success) {
      ...
      // Hapa tunajenga muonekano wa majibu kutoka Sightengine
      result.innerHTML = `
        <div style="text-align: left;">
          <p>✅ <b>Scan Imekamilika!</b></p>
