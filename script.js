...
async function startScan() {
  ...
  btn.disabled = true;
  result.innerHTML = "🔄 Inachakata picha... tafadhali subiri...";
  ...
  try {
    ...
    // 4. Tuma ombi kwenye API yetu ya Vercel
    ...
    if (resultData.success) {
      ...
      // Hapa ndipo tunatumia data zako mpya:
      const isSafe = ai.nudity?.none > 0.9;
      const isReal = ai.type?.ai_generated < 0.5;
      const faceCount = ai.faces ? ai.faces.length : 0;

      result.innerHTML = `
        <div style="text-align: left; padding: 15px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 8px;">
          <h3 style="margin-top:0;">✅ Scan Imekamilika!</h3>
          <p><b>Hali ya Picha:</b> ${isSafe ? "✅ Salama" : "⚠️ Maudhui yanayotiliwa shaka"}</p>
          <p><b>Uhalisi:</b> ${isReal ? "✅ Picha ya kweli" : "⚠️ Inaonekana kama AI"}</p>
          <p><b>Nyuso zilizogundulika:</b> ${faceCount}</p>
          
          <details style="margin-top:10px; cursor:pointer;">
            <summary>Angalia maelezo yote (Raw Data)</summary>
            <pre style="font-size: 10px; overflow-x: auto; background:#eee; padding:5px;">
