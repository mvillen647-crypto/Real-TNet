...
async function startScan() {
  ...
  if (activeScanType !== "image") {
    result.innerHTML = "⚠️ Kwa sasa mfumo huu unakubali picha pekee.";
    return;
  }
  ...
  btn.disabled = true;
  result.innerHTML = "🔄 Inapakia picha kwenye GitHub... tafadhali subiri.";
  ...
  try {
    ...
    reader.onload = async (e) => {
      const base64Content = e.target.result.split(",")[1]; // Ondoa data:image/...

      // 2. Tuma picha kwenye GitHub (Upload to /uploads folder)
      // Kumbuka: Hii inahitaji GitHub Token (usiiweke wazi kwenye code)
      const fileName = `upload_${Date.now()}.jpg`;
      const response = await fetch(`https://api.github.com/repos/MWENYE_REPO/JINA_LA_REPO/contents/uploads/${fileName}`, {
        method: "PUT",
        headers: {
          "Authorization": "token YOUR_GITHUB_PAT", // Hapa weka token yako kwa usalama
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          message: `Add new scan: ${fileName}`,
      ...
      if (response.ok) {
        result.innerHTML = `
          ✅ <b>Picha imepakiwa!</b><br>
          GitHub Action itaanza kuscan picha hii muda mfupi ujao.<br>
          Matokeo yataonekana kwenye folda ya /results baada ya sekunde chache.
        `;
      } else {
        throw new Error("Imeshindikana kupakia picha kwenye GitHub.");
      }
